...
1. **Is the tiered convention present?** Glob for `docs/LLM_MAP.md` and `docs/STRUCTURE.md`.
   ...
   - Both (or `LLM_MAP.md`) exist → the convention is established → **go to STEP 2B (Improve).**
...
1. **Scaffold the scope contract.** Copy the bundled template `assets/STRUCTURE.template.md` (next to this
   ...
   they must not drift.
2. **Seed the L0 index.** Create `docs/LLM_MAP.md` with a title and a short "what this is" line. Leave the
   per-system entries for the write cycle to fill — don't hand-write the whole map now.
3. **Do NOT pre-build empty `docs/<Area>/` folders or stub READMEs.** A synthesis-less stub README is exactly
   what an L1 must never be. Let `doc-maintenance` create each area's docs as it actually documents it.
4. **Hand off to `doc-maintenance`.** Invoke that skill to run the **inventory cycle** (its per-iteration
...
1. **Produce a short state report first** so the user sees the shape of the work before you spend on it. Use
   the `codebase-context` skill's grep-first protocol to surface, across the existing docs:
